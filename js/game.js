...
var game = {
	...
	setHandlers: function () {
		...
		$(window)
			...
			.on('hashchange', function () {
				...
				$('#tweet iframe').remove();
				var html =
					'<a href="https://twitter.com/share" class="twitter-share-button"{count} data-url="http://flexboxfroggy.com" data-via="thomashpark">Tweet</a> ' +
	...
	loadLevel: function (level) {
		...
		for (var i = 0; i < string.length; i++) {
			...
			var logoImage = $('<div/>').addClass(
				'frog bg animated pulse infinite logo-' + logo
			);
	...
	check: function () {
		...
		var correctAnswer = level.answer
			.trim()
			.toLowerCase()
			.replace(/["']/g, '')
			.replace(/ /g, '')
